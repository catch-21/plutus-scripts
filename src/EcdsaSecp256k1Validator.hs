...
module EcdsaSecp256k1Validator (writeSerialisedScript) where
...
mkPolicy red _ =
  case PlutusV2.fromBuiltinData red of
    ...
    Just (vkey, msg, sig) ->
      if BI.verifyEcdsaSecp256k1Signature vkey msg sig
        then ()
        else P.traceError "Trace error: ECDSA validation failed"
...
writeSerialisedScript = void $ writeFileTextEnvelope "ecdsa-secp256k-policy.plutus" Nothing serialisedScript
